...
int main(int argc, char** argv) {
	...
	struct bpf_object* bpf_obj_off = nullptr;
	...
	util::load_xdp_prog("build/bpf/xdp_comp_off_kern_nic.o", &bpf_obj_off, &prog_fd, iface_index, xdp_flags_nic);	
	util::attach_xdp_fd(iface_index, prog_fd, xdp_flags_nic);

	auto do_gpv_telemetry_map_fd = util::find_map_fd(bpf_obj_off, "do_telemetry_map");

	auto counter_map_fd = util::find_map_fd(bpf_obj_off, "counter_map");
	...
	xdp_router xdp_router(xdp_router_map_fd, bpf_obj, bpf_obj_off);
	...
	set_tmt_hkey.proto = 6; //TCP

	xdp_set_gpv_telemetry.add_source(set_tmt_hkey);

	set_tmt_hkey.proto = 17; //UDP

	xdp_set_gpv_telemetry.add_source(set_tmt_hkey);	

	set_tmt_hkey.proto = 1; //ICMP
